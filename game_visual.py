...
def generate_text(
    text, fg, bg, rect_center, border=None, border_width=5, font=generic_scalable_font
):
...
class HexTile:
    ...
    def place(self, x, y):
        ...
        self.tile = pygame.draw.polygon(
            screen,
            BLUE,
            [
                (
                    self.x + HEX_SIZE * math.cos(math.radians(angle)),
                    self.y + HEX_SIZE * math.sin(math.radians(angle)),
                )
        ...
        for angle in range(0, 360, 60):
            ...
            pygame.draw.polygon(
                screen,
                self.colors[self.composition[index]],
                [
                    (self.x, self.y),
                    (
                        self.x + (HEX_SIZE - 4) * math.cos(math.radians(angle)),
                        self.y + (HEX_SIZE - 4) * math.sin(math.radians(angle)),
                    ),
                    (
                        self.x + (HEX_SIZE - 4) * math.cos(math.radians(angle + 60)),
                        self.y + (HEX_SIZE - 4) * math.sin(math.radians(angle + 60)),
                    ),
                ],
            )
            ...
            pygame.draw.polygon(
                ...
                [
                    ...
                    (
                        self.x + (HEX_SIZE) * math.cos(math.radians(angle + 60)),
                        self.y + (HEX_SIZE) * math.sin(math.radians(angle + 60)),
                    ),
                ],
                width=3,
            )
            # self.plots[index].center= (
            #     self.x
            #     + (HEX_SIZE / 2)
            #     * math.cos(math.radians(angle + 30 + 60 * self.rotation)),
            #     self.y
            #     + (HEX_SIZE / 2)
            #     * math.sin(math.radians(angle + 30 + 60 * self.rotation)),
            # )
        ...
        for angle, plot in zip(range(0, 360, 60), self.plots):
            ...
            screen.blit(
                ...
                (
                    (
                        self.x
                        + (HEX_SIZE - HEX_SIZE / 3) * math.cos(math.radians(angle + 30))
                        - tile_letter.get_width() / 2,
                        self.y
                        + (HEX_SIZE - HEX_SIZE / 3) * math.sin(math.radians(angle + 30))
                        - tile_letter.get_height() / 2,
                    )
                ),
            )
    ...
    def adjacency_debug(self):
        ...
        for angle in range(0, 360, 60):
            index = angle // 60
            adjaceny_value = hex_font.render(
                str(int(self.adjacencies[index])), True, BLACK
            )
            screen.blit(
                adjaceny_value,
                (
                    (
                        self.x
                        + (HEX_SIZE - HEX_SIZE / 4) * math.cos(math.radians(angle + 30))
                        - adjaceny_value.get_width() / 2,
                        self.y
                        + (HEX_SIZE - HEX_SIZE / 4) * math.sin(math.radians(angle + 30))
                        - adjaceny_value.get_height() / 2,
                    )
                ),
            )
...
class HexGrid:
    ...
    def draw_hex_grid(self, rows, cols, hex_size, tiles):
        ...
        for row in range(rows):
            for col in range(cols):
                ...
                y = (
                    row * hex_size * math.sqrt(3)
                    + (hex_size * math.sqrt(3) / 2 * (col % 2))
                    + offset_y
                )
        ...
        for row in range(rows):
            for col in range(cols):
                ...
                tile: HexTile = self.grid[row][col]
    ...
    def detect_adjacencies(self, row, col):
        ...
        for plot_adjacent, plot in zip(adjacency_table, tile.plots):
            ...
            adjacent_plots = [(value, (index + 1) % 6), (value, (index - 1) % 6)]
            if plot_adjacent:
...
class InfoBar:
    def __init__(self, players: list):
        ...
        self.round_btn = pygame.Rect(
            WIDTH * 2 / 5, 4.5 * HEIGHT / 6, 2 * WIDTH / 10, 0.3 * HEIGHT / 6
        )
        self.round_num = 1
        ...
        self.resource_icons = (
            (
                ...
                pygame.transform.scale(
                    pygame.image.load("assets/droplet.png"),
            ...
            (
                "food",
                pygame.transform.scale(
                    pygame.image.load("assets/food.png"),
                    (WIDTH * width_scale, HEIGHT * height_scale),
                ),
            ),
            (
                "wood",
                pygame.transform.scale(
                    pygame.image.load("assets/log.png"),
                    (WIDTH * width_scale, HEIGHT * height_scale),
                ),
            ),
            (
                "brick",
                pygame.transform.scale(
                    pygame.image.load("assets/stone.png"),
                    (WIDTH * width_scale, HEIGHT * height_scale),
                ),
            ),
        )
        ...
        self.die_icons = {
            4: pygame.transform.scale(
                pygame.image.load("assets/dice/d4.png"),
                (WIDTH * width_scale, HEIGHT * height_scale),
            ),
            6: pygame.transform.scale(
                pygame.image.load("assets/dice/d6.png"),
                (WIDTH * width_scale, HEIGHT * height_scale),
            ),
            8: pygame.transform.scale(
                pygame.image.load("assets/dice/d8.png"),
                (WIDTH * width_scale, HEIGHT * height_scale),
            ),
            12: pygame.transform.scale(
                pygame.image.load("assets/dice/d12.png"),
                (WIDTH * width_scale, HEIGHT * height_scale),
            ),
            20: pygame.transform.scale(
                pygame.image.load("assets/dice/d20.png"),
                (WIDTH * width_scale, HEIGHT * height_scale),
            ),
        }
    ...
    def draw_sections(self):
        ...
        round_num_box = [
            (WIDTH / 2 - WIDTH / 20, 5.1 * HEIGHT / 6 - HEIGHT / 25),
            (WIDTH / 10, 0.2 * HEIGHT / 6),
        ]
        ...
        for n in range(4):
            pygame.draw.rect(
                ...
                BLACK,
                [
                    (main_box[0][0], main_box[0][1] + n * HEIGHT / 5),
                    (main_box[1][0], main_box[1][1] / 4),
                ],
                width=1,
            )
            ...
            screen.blit(
                player.name,
                (WIDTH / 20 + offset[0], HEIGHT / 10 + n * HEIGHT / 5 + offset[1]),
            )
            pygame.draw.circle(
                screen,
                color=player.color,
                radius=WIDTH / 300,
                center=(
                    WIDTH / 20 + 5.5 * offset[0],
                    HEIGHT / 10 + n * HEIGHT / 5 + 3 * offset[1],
                ),
            )
            ...
            for index, icon in enumerate(self.resource_icons):
                screen.blit(
                    icon[1],
                    (
                        WIDTH / 20 + 2 * offset[0] + index * 7 * offset[0],
                        HEIGHT / 10
                        + (n) * HEIGHT / 5
                        + 8 * (index // 4 + 1) * offset[1],
                    ),
                )

                value_text, value_rect = generate_text(
                    ": " + str(player.resources[icon[0]]),
                    BLACK,
                    WHITE,
                    (
                        WIDTH / 20
                        ...
                        + index * 7 * offset[0]
                        + WIDTH / 45,
                        HEIGHT / 10 + (n) * HEIGHT / 5 + 8 * offset[1] + HEIGHT / 100,
                    ),
                )
            ...
            for index, die in enumerate(player.dice):
                screen.blit(
                    self.die_icons[die.value],
                    (
                        WIDTH / 20 + 2 * offset[0] + index * 5 * offset[0],
                        HEIGHT / 10 + (n) * HEIGHT / 5 + HEIGHT / 5 - 10 * offset[1],
                    ),
                )
        ...
        text, text_rect = generate_text(
            "Play Round", BLACK, GREEN, self.round_btn.center
        )
        screen.blit(text, text_rect)
        round_value, round_value_rect = generate_text(
            f"Round {self.round_num}", BLACK, BROWN, pygame.Rect(round_num_box).center
        )
...
class GameEngine:
    ...
    def __init__(self, game, tiles, grid: HexGrid):
        self.players = [
            Player(1, RED, self),
            Player(2, PURPLE, self),
            Player(3, ORANGE, self),
            Player(4, YELLOW, self),
        ]
        self.assign_players()
        start_pos_set = False
        self.game = game
        ...
        self.quit_button = pygame.Rect(
            9 / 10 * WIDTH, HEIGHT / 30, WIDTH / 12, HEIGHT / 20
        )
        self.quit_text, self.quit_rect = generate_text(
            "Quit Sim", BLACK, GREEN, self.quit_button.center
        )
